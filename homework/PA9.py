...
def optimize_wings(price_list,num_wings):
    prc = []
    prc.append(Order(0,0))
    index=0

    for i in range(0,len(price_list)):
        while index < price_list[i].wings:
            prc.append(price_list[i])
            index+=1

    while index < num_wings:
        prc.append(Order(0,10000))
        index+=1
    ...
    for j in range(1, num_wings+1):
        ...
        for i in range(1, j + 1):
            if best > prc[i].price + rev[j - i] or best ==0:
    ...
    result=[]
    while num_wings>0:
        result.append(prc[split[num_wings]])
