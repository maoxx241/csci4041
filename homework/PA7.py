...
def insert_floor(floor_tree,floor):
    ...
    if y==None:
        ...
        if floor.floor_num<y.floor_num:
            y.left = floor
        else:
            y.right = floor
...
def find_nearest_unhaunted(floor_tree,floor_num):
    x=floor_tree.root
    while x != None and floor_num != x.floor_num:
        if floor_num<x.floor_num:
            if x.left!=None:
                x=x.left
            else:
                if abs(x.parent.floor_num-floor_num)<abs(x.floor_num-floor_num):
                    return x.parent
                else:
                    return x
        else:
